...
def apply_network_restrictions(username: str, whitelist_file: str) -> bool:
    """Apply network restrictions for a user using whitelist file.
    ...
    """
    try:
        # Load domains from whitelist file
        domains = set()
        ...
        # Create network restrictor instance
        restrictor = NetworkRestrictor(username)
        
        # Apply restrictions
        restrictor.apply_restrictions(domains, set())
        
        print(f"✅ Network restrictions applied for user '{username}'")
        return True
        
    except Exception as e:
        print(f"❌ Failed to apply network restrictions: {e}")
        return False


def remove_network_restrictions(username: str) -> bool:
    ...
    try:
        ...
        # Remove restrictions
        restrictor.remove_restrictions()
        
        print(f"✅ Network restrictions removed for user '{username}'")
        return True
